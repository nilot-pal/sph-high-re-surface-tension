    ...
    interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < FluidWallData::contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != wall_neighborhood.current_size_; ++n)
        {
        	...
        	//Fluid fluid_;
        	Real c_i = 20; //dummy value
        	Real c_j = 20; //dummy value
        	Real p_star = (this->rho_[index_i]*c_i*p_in_wall+ rho_k*c_j*this->p_[index_i])/(this->rho_[index_i]*c_i + rho_k*c_j);
        	p_star = SMAX(Real(0), p_star + 0.5*this->riemann_solver_.DissipativePJump(u_jump));
        	if (p_star > this->p_[index_i]) {
        		Real gamma = 7.0;
        		q_i = sqrt(1 + (gamma+1)*(p_star/this->p_[index_i] -1)/(2*gamma));
        	}
        	if (p_star > p_in_wall) {
        		Real gamma = 7.0;
        		q_j = sqrt(1 + (gamma+1)*(p_star/p_in_wall -1)/(2*gamma));
        	}
        	Real u_L = this->vel_[index_i].dot(n_k[index_j]);
			Real u_R = (-this->vel_[index_i] + 2*vel_ave_k[index_j]).dot(n_k[index_j]);
			StdVec<Real>UHllc = this->riemann_solver_.UHllc(this->rho_[index_i], rho_k, this->p_[index_i], p_in_wall, this->vel_[index_i] , vel_ave_k[index_j], u_L, u_R, c_i, c_j, q_i, q_j);
        	Vecd vel_Hllc = Vecd::Zero();
        	vel_Hllc[0] = UHllc[1];
        	vel_Hllc[1] = UHllc[2];
        	Real u_hllc = vel_Hllc.dot(n_k[index_j]);
        	//acceleration -= 2*this->getPressure(UHllc[0]) * dW_ijV_j * e_ij;
        	// EOS from Monaghan (1994) -> P_i = rho_0_i*c_i^2/gamma*[(rho_i/rho_0_i)^gamma -1]
        	Real gamma_new = 1.4;
        	Real rho_ref = 1.0;
        	Real pHllc = rho_ref*pow(1,2)/gamma_new*(pow(UHllc[0]/rho_ref , gamma_new) -1);
        	//Real pHllc = 1.0;
        	acceleration -= 2 * pHllc * dW_ijV_j * e_ij; 
        	Real h_custom = 1.3*0.025; // from 2d dambreak example
    ...
    interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < FluidWallData::contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != wall_neighborhood.current_size_; ++n)
        {
            ...
            //density_change_rate += 2.0*(this->vel_[index_i] - vel_in_wall).dot(e_ij) * dW_ijV_j;
            Real u_L = this->vel_[index_i].dot(n_k[index_j]);
